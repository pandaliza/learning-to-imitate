"""M9-ATTACH trainer: train_pi05_m9.py + a SELECTABLE ATTACH POINT (--intent-flow-attach).

PURELY ADDITIVE over train_pi05_m9.py: every existing arm (M1-M8, M9) runs unchanged, and the new flag
defaults to `penult` = exactly what train_pi05_m9.py does today. Only `--intent-flow-attach vl` is new.

  penult (default) -- head reads the ACTION EXPERT's penultimate hidden [B,H,1024]. Intent and action
                      share the whole trunk; the aux gradient shapes the action expert AND, through it,
                      the VLM.
  vl               -- head reads the LIVE PaliGemma prefix image-token mean [B,2048]. The action expert
                      is NOT touched by the aux gradient; only the VLM (LoRA adapters + SigLIP vision
                      tower) is shaped. Max's phrasing taken literally ("a flow head that conditions on
                      VL reps") and the no-sharing end of Sarvesh's shared-vs-separate question.

Verified prerequisite: pi05_base_nointent uses gemma_2b_lora -> _set_lora_requires_grad freezes ONLY the
PaliGemma LLM base ([freeze-lora] trainable=1127.8M frozen=2508.5M). The prefix LoRA adapters and the
whole vision tower ARE trainable, so an aux loss on VL reps is NOT a no-op.

CAVEAT for the ablation table: the two taps shape DIFFERENT parameter sets, so read this as "where does
the supervision bite", not "same experiment, different tap".

Copy rather than edit because a live M9 run trains out of train_pi05_m9.py and would pick it up on
resume; fold the forks back together once the arm settles.

  python examples/openpi/train_pi05_m9_attach.py \
    --pi05-config pi05_base_nointent --slot-task-config libero_goal_suite_image_slot_intent_vl \
    --pi05-weights .../pi05_base_pytorch \
    --intent-flow --intent-flow-attach vl --intent-flow-decoder flow --intent-flow-weight 0.25 \
    --steps 30000 --batch-size 2 --accum-steps 4 --out .../pi05_m9_attach_vl

--- inherited: M9 trainer: pi0.5 + the STRIPPED intent head (no workspace token, no CFG, no JEPA).
...
def main():
    ...
    ap.add_argument("--intent-flow-attach", default="penult", choices=["penult", "vl"],
                    help="M9: tap the action-expert penultimate (penult, default = train_pi05_m9.py's "
                         "behaviour) or the live PaliGemma prefix image-token mean (vl)")
    ...
    if aux_intent:
        ...
        # --intent-flow-attach vl instead taps the PaliGemma prefix, whose width is the VLM hidden size.
        _expert_width = int(raw_model.action_out_proj.in_features)
        if args.intent_flow_attach == "vl":
            # PaliGemma prefix width, read off the live module (the path pi0_pytorch.vl_image_features
            # itself uses) rather than a config attribute, so it can't silently drift. LIBERO: 2048.
            _expert_width = int(
                raw_model.paligemma_with_expert.paligemma.language_model.layers[0].self_attn.q_proj.in_features)
        _intent_dim = int(ds[0]["wsm_intent_target"].shape[-1])
        ...
        if is_main:
            print(f"[M9-INTENT] attach={args.intent_flow_attach} tap_dim={_expert_width} "
                  f"intent_dim={_intent_dim} decoder={args.intent_flow_decoder} "
                  f"target={args.intent_flow_target} weight={args.intent_flow_weight} "
                  f"(no w, no CFG, no JEPA)", flush=True)
    else:
    ...
    while step < args.steps:
        ...
        for batch in loader:
            ...
            act_hidden = vl_tap = None
            if aux_intent and not args.aux_condition:
                action_loss, vl_mean = model(observation, actions, return_vl_mean=True)  # single VL pass
            elif intent_flow and args.intent_flow_attach == "vl":
                # ATTACH=vl: take the prefix image-token mean from THIS SAME DDP forward (B, 2048).
                # A second raw_model.vl_image_features() pass would hit the LoRA params with two DDP grad
                # hooks per step -> "Expected to mark a variable ready only once". The forward's own
                # docstring calls this out; M4 uses the same route. Grad flows (no no_grad here), so the
                # aux loss still reaches the VLM -- and we avoid paying for a second VL forward.
                action_loss, vl_tap = model(observation, actions, return_vl_mean=True)
            ...
            elif intent_flow:  # M9: reps -> intent. No w read anywhere in this arm.
                itgt = batch["wsm_intent_target"].to(device).float()   # (B, h*eef_dim) or (B, eef_dim)
                if args.intent_flow_attach == "vl":
                    # vl_tap = live prefix image-token mean from the DDP forward above (grad ON -> shapes
                    # the VLM's LoRA adapters + vision tower). NOT the cached VL grids, which are detached
                    # fp16 arrays the aux loss could never reach.
                    # [B, 2048] -> [B, 1, 2048] for the head's [B, T, D] contract (its mean over T is then
                    # a no-op).
                    tap = vl_tap.unsqueeze(1)
                else:
                    tap = act_hidden                                   # ATTACH=penult (default, = M9)
                _, if_total, if_m = intent_flow_head(tap, itgt, global_step=step)
            ...
            if intent_flow and args.intent_flow_attach == "vl" and step == 0 and micro == 0 and is_main:
                # ATTACH=vl only pays off if the aux loss actually reaches the VLM. A detached tap (e.g.
                # cached VL grids) or an all-frozen prefix would train the head happily while changing
                # NOTHING upstream -- a no-op that looks healthy. Assert once, at step 0, before the run
                # burns 4 GPUs x 48h. Counts params upstream of the tap carrying non-zero grad.
                _up = [(n, p) for n, p in raw_model.named_parameters()
                       if p.requires_grad and ("lora_" in n or "vision" in n or "img" in n)]
                _hit = [n for n, p in _up if p.grad is not None and p.grad.abs().sum().item() > 0]
                print(f"[M9-ATTACH check] VLM params w/ non-zero grad: {len(_hit)}/{len(_up)}", flush=True)
                if not _hit:
                    raise RuntimeError(
                        "ATTACH=vl is a NO-OP: no gradient reached any VLM parameter. The tap is "
                        "detached or the prefix is frozen -- do not waste the allocation.")
                print(f"[M9-ATTACH check] OK, e.g. {_hit[0]}", flush=True)
