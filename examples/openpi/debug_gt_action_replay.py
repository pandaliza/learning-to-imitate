"""GT-action replay diagnostic: test if raw demo actions produce success in the RoboCasa env.

This is the highest-priority diagnostic. If raw demo actions fail to produce success,
the bug is in env/controller/action-space wiring (not the policy).
If they succeed, the bug is in policy-side: obs construction, normalization, or model.
...
def main():
    ...
    print(f"[GT Replay] Loading demo episodes for {args.task}", flush=True)
    ...
    try:
        env = robosuite.make(
            ...
            use_camera_obs=False,  # No images for speed
            camera_names=[],
            control_freq=20,
            horizon=1000,
        )
    ...
    print(f"[GT Replay] Found {len(demo_episodes)} demo episodes, replaying first {len(demo_episodes)}", flush=True)
            ...
            try:
                ...
                if done:
                    ...
                    if "success" in info:
                        ...
                        try:
                            success = bool(env.is_success(obs))
                        except:
                            success = False
                    print(f"[Episode {ep_idx}] Final success: {success}")
                    break
    ...
    total = len(results)
    print(f"\n[GT Replay Summary] {successes}/{total} demos replayed successfully")
    ...
    print(f"[GT Replay] Results saved to {args.out}")
