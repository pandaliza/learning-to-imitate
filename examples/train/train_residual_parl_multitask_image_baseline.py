"""Multi-task Residual PARL on top of a frozen *baseline* (non-intent) image policy.

Same as train_residual_parl_multitask_image.py but uses `TrainingAgent` (plain
flow-matching BC) as the base policy instead of `FlowIntentAgent`. This is the
"RL on top of BC" comparison that isolates the contribution of the intent
architecture vs online RL fine-tuning.

Key differences from the flow-intent version:
  - Uses `TrainingAgent`, not `FlowIntentAgent`
  - `agent.sample(act_0, obs, ...)` needs initial Gaussian noise
  - Same encoder_ema attribute exists on both, so PARL obs extraction is identical

Usage:
    python examples/train_residual_parl_multitask_image_baseline.py \
        task=libero_object_suite_image \
        residual_parl.flow_intent_ckpt=/path/to/baseline_ckpt.pt
"""
...
def _build_obs_tensordict(obs_raw, dataset, image_obs_keys, num_envs, device):
    """Pack env obs (dict of numpy arrays) into a TensorDict."""
    state_norm = dataset.normalizer["obs"]["state"].normalize(
        obs_raw["state"].astype(np.float32)
    )
    obs_dict = {
        "state": torch.tensor(state_norm, device=device, dtype=torch.float32),
    }
...
@torch.no_grad()
def _compute_obs_emb(base_agent, obs_td):
    """Run the frozen encoder to get embeddings. Returns (B, obs_steps, emb_dim)."""
...
@torch.no_grad()
def _sample_base_action(base_agent, obs_td, horizon, act_dim, num_steps, device):
    """Sample an action chunk from the baseline (flow matching) policy."""
    B = obs_td.batch_size[0] if hasattr(obs_td.batch_size, "__len__") else obs_td.batch_size
    act_0 = torch.randn(B, horizon, act_dim, device=device)
    action = base_agent.sample(
        act_0=act_0, obs=obs_td, num_steps=num_steps, use_ema=True,
    )
    return action  # (B, horizon, act_dim)
...
def evaluate_multitask(
    ...
    dataset,
    config: Config,
    base_agent: TrainingAgent,
    image_obs_keys: list[str],
    flow_num_steps: int = 9,
    ...
    for task_name, bddl_file in tasks:
        ...
        for _ in range(num_episodes_per_task // num_envs):
            ...
            while t < config.task.max_episode_steps:
                obs_td = _build_obs_tensordict(obs_raw, dataset, image_obs_keys, num_envs, device)

                base_actions_norm = _sample_base_action(
                    base_agent, obs_td, horizon, act_dim, flow_num_steps, device,
                )
                obs_emb = _compute_obs_emb(base_agent, obs_td)
        ...
        envs.close()
        mean_s = float(np.mean(task_success))
    ...
    metrics = {
        "eval/mean_success": float(np.mean(all_success)),
        "eval/mean_reward": float(np.mean(all_success)),
...
def main(raw_cfg: DictConfig) -> None:
    ...
    assert image_obs_keys, "This script is for image obs — image_obs_keys must be set"
    loguru.logger.info(f"Image obs keys: {image_obs_keys}")

    # ---- Frozen baseline (TrainingAgent, not FlowIntentAgent) -----------
    base_ckpt = parl_cfg_dict["flow_intent_ckpt"]
    assert base_ckpt, "residual_parl.flow_intent_ckpt must be set (baseline checkpoint)"

    # For image obs, obs_dim is the encoder emb_dim.
    ...
    loguru.logger.info(f"Frozen BC baseline loaded from {base_ckpt}")

    # ---- PARL agent: obs = flattened encoder embeddings ----------------
    ...
    loguru.logger.info(
        f"PARL obs_dim = obs_steps * emb_dim = {config.task.obs_steps} * {emb_dim} = {obs_dim_flat}"
    )
    ...
    resume_ckpt = parl_cfg_dict.get("resume_ckpt", "")
    resume_global_step = int(parl_cfg_dict.get("resume_global_step", 0))
    ...
    if resume_ckpt:
        agent.load(resume_ckpt, load_optimizers=False)
        loguru.logger.info(
            f"Resumed PARL agent weights from {resume_ckpt} "
            f"(resume_global_step={resume_global_step}, note: buffer & optimizer state NOT restored)"
        )
    ...
    loguru.logger.info(f"Starting image multi-task training (BC base) for {total_env_steps} env steps")
    ...
    obs_raw, _ = envs.reset()
    obs_td = _build_obs_tensordict(obs_raw, dataset, image_obs_keys, num_envs, device)
    base_actions_norm = _sample_base_action(base_agent, obs_td, horizon, act_dim, flow_num_steps, device)
    obs_emb = _compute_obs_emb(base_agent, obs_td)
    ...
    while global_step < total_env_steps:
        ...
        for ei in range(num_envs):
            ...
            if global_step < warmup_steps and not resume_ckpt:
                ...
                a_exec, delta, _lp = agent.sample_action(obs_emb_flat[ei], base_flat)
                all_a_exec.append(a_exec)
                all_delta.append(delta)
        ...
        # --- Next obs embeddings + base ---
        next_obs_td = _build_obs_tensordict(next_obs_raw, dataset, image_obs_keys, num_envs, device)
        next_base_norm = _sample_base_action(base_agent, next_obs_td, horizon, act_dim, flow_num_steps, device)
        next_obs_emb = _compute_obs_emb(base_agent, next_obs_td)
        next_base_chunk = next_base_norm[:, start_idx:end_idx, :].cpu().numpy()
        next_obs_emb_flat = next_obs_emb.reshape(num_envs, -1).cpu().numpy()
        ...
        # Switch task when all envs have finished at least one episode
        ...
        obs_emb_flat = next_obs_emb_flat
        ...
        if global_step % eval_freq < act_steps * num_envs:
            eval_metrics = evaluate_multitask(
                agent, tasks, dataset, config, base_agent, image_obs_keys,
