"""Multi-task Residual PARL on top of a frozen *image-based* flow-intent policy.

Differences from the state-based multi-task script:
  - Env returns dict obs: {"state": (...), "agentview_rgb": (...), "eye_in_hand_rgb": (...)}
  - Obs is packed into a TensorDict before being passed to flow_intent
  - PARL actor/critic see *flattened frozen-encoder embeddings* — not raw pixels.
    obs_emb = flow_intent.encoder_ema(obs_dict)  # (B, obs_steps, emb_dim)
    PARL obs = obs_emb.flatten(1)                 # (B, obs_steps * emb_dim)
  - Replay buffer stores embeddings (1024 dims) instead of raw obs.

Usage:
    python examples/train_residual_parl_multitask_image.py \
        task=libero_object_suite_image_flow_intent \
        residual_parl.flow_intent_ckpt=/path/to/image_flow_intent.pt
"""
...
def _build_obs_tensordict(obs_raw, dataset, image_obs_keys, num_envs, device):
    """Pack env obs (dict of numpy arrays) into a TensorDict for flow_intent."""
...
@torch.no_grad()
def _compute_obs_emb(flow_intent, obs_td):
    """Run the frozen encoder to get embeddings. Returns (B, obs_steps, emb_dim)."""
    obs_emb = flow_intent.encoder_ema(obs_td, None)
    if obs_emb.dim() == 2:
        obs_emb = obs_emb.unsqueeze(1)
    return obs_emb  # (B, obs_steps, emb_dim)
    ...
    for task_name, bddl_file in tasks:
        ...
        for _ in range(num_episodes_per_task // num_envs):
            ...
            while t < config.task.max_episode_steps:
                ...
                with torch.no_grad():
                    base_actions_norm = flow_intent.sample(obs_td, use_ema=True, num_steps=flow_num_steps)
                    obs_emb = _compute_obs_emb(flow_intent, obs_td)
...
def main(raw_cfg: DictConfig) -> None:
    ...
    # For image obs, obs_dim is the encoder emb_dim (consistent with train_libero.py).
    config.task.obs_dim = config.network.emb_dim

    flow_intent = FlowIntentAgent(config)
    flow_intent.load(flow_intent_ckpt)
    for mod in [flow_intent.encoder, flow_intent.intent_flow_map, flow_intent.encoder_ema]:
    ...
    loguru.logger.info(f"Starting image multi-task training for {total_env_steps} env steps")
    ...
    while global_step < total_env_steps:
        ...
        with torch.no_grad():
            next_base_norm = flow_intent.sample(next_obs_td, use_ema=True, num_steps=flow_num_steps)
            next_obs_emb = _compute_obs_emb(flow_intent, next_obs_td)
        ...
        if global_step % eval_freq < act_steps * num_envs:
            eval_metrics = evaluate_multitask(
                agent, tasks, dataset, config, flow_intent, image_obs_keys,
